...
/**
 * @author leix.xie
 * @since 2019/8/24 01:27
 */
public class JarStorePathUtil {
    private static final Logger logger = ElfLoggger.getLogger();

    private static final String STORE_PATH = ElfStore.getStorePath("elf_tomcat_webapp");
    ...
    public static String getJarLibPath() {
        return new File(getJarStorePath(), getMavenInfo().getSpringBootLib()).getPath();
    }

    public static String getJarSourcePath() {
        return new File(getJarStorePath(), getMavenInfo().getSpringBootClasses()).getPath();
    }
