...
/**
 * @edit leix.xie
 * 由于agent需要多线程使用cache，所以将classBytesCache修改为public，同时，在读取和写时添加锁
 */
public class Enhancer implements ClassFileTransformer {
    ...
    public static void dumpClassIfNecessary(String className, byte[] data, EnhancerAffect affect) {
        ...
        final File dumpClassFile = new File("/tmp/elf-class-dump/" + className + ".class");
    ...
    private static boolean isSelf(Class<?> clazz) {
        return clazz != null
                && (isLoadByElfClassLoader(clazz) || isLoadByMagicClassLoader(clazz));
    }

    private static boolean isLoadByElfClassLoader(Class<?> clazz) {
