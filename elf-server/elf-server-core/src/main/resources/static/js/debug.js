$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (ret) {
                if (ret.status === 0) {
                    ...
                    elf.error('获取应用列表失败，' + ret.message);
                }
            }
        ...
        $.ajax({
            ...
            success: function (ret) {
                if (ret.status == 0) {
                    ...
                    elf.error('获取应用列表失败, 错误信息：' + ret.message);
                }
            }
        });
    }
    ...
    function handleResult(content) {
        ...
        if (resType == "qdebugadd") {
            ...
            if (res.code == 0) {
                ...
                elf.success("断点添加成功，请触发断点")
            } else {
                enableBreakPoint();
                keepRunning = false;
                elf.error("断点添加失败, " + res.message)
            ...
            if (res.code == 0) {
                ...
                elf.warning(res.message + "，代码查看仅可通过反编译")
                console.log(res.message);
            }
            getAllClass();
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status == 0) {
                    ...
                    elf.warning("release info 解析失败，" + res.message + "。代码查看仅可通过反编译")
                }
            },
            error: function (error) {
                elf.warning("release info 解析失败，" + error.message + "。代码查看仅可通过反编译")
            }
        })
    }
    ...
    $("#add-breakpoint").click(function () {
        ...
        if (currentFile.lastIndexOf(".java") != currentFile.length - 5) {
            elf.error("在线 Debug 仅支持 Java 文件");
            return;
        }
        ...
        if (line == null || line == undefined || line == "" || line < 0) {
            if (decompilerFile) {
                elf.error("找不到源文件对应行号，请重新选择行号");
            } else {
                elf.error("请选择需要添加断点的代码行")
            }
            return;
        } else {
            currentPoint = {
    ...
    function searchVar() {
        ...
        if (varSearchResult.length === 0) {
            elf.warning('未找到名字为[' + searchVarName + ']的变量');
        } else {
            varSearchResult[varSearchResultIndex].get(0).scrollIntoView({behavior: "smooth"});
