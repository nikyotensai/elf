$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (ret) {
                if (ret.status == 0) {
                    ...
                    elf.error('获取主机列表失败, 错误信息：' + data.message);
                }
            }
        });
    ...
    function getHostInfo() {
        elfWS.send(currentHost, 10, "", "", stop, handleResult)
        //send(currentHost, 10, "");
    }

    function getAllThreads() {
        elfWS.send(currentHost, 11, "0@-1@-1", {
            type: 0,
            maxDepth: -1,
            threadId: -1
    ...
    function getThreadDetail(threadId) {
        ...
        elfWS.send(currentHost, 11, "1@" + threadId + "@" + maxDepth, {
            type: 1,
            maxDepth: maxDepth,
            threadId: threadId
        }, keepRunningFun, handleResult)
    ...
    function getThreadDump() {
        ...
        elfWS.send(currentHost, 11, "2@-1@" + maxDepth, {
            type: 2,
            maxDepth: maxDepth,
            threadId: -1
        }, keepRunningFun, handleResult)
    ...
    function getDeadLock() {
        ...
        elfWS.send(currentHost, 11, "3@-1@" + maxDepth, {
            type: 3,
            maxDepth: maxDepth,
            threadId: -1
        }, keepRunningFun, handleResult)
    ...
    function getHeapHisto(timestamp) {
        ...
        if (timestamp) {
            elfWS.send(currentHost, 12, "heaphisto " + param + " " + timestamp, {
                param: param,
                timestamp: timestamp
            }, keepRunningFun, handleResult);
            //send(currentHost, 12, "heaphisto " + param + " " + timestamp);
        } else {
            elfWS.send(currentHost, 12, "heaphisto " + param + " " + -1, {
                param: param,
                timestamp: -1
            }, keepRunningFun, handleResult)
    ...
    function getJarInfo() {
        var command = "jarinfo";
        elfWS.sendCommand(currentHost, 13, command, keepRunningFun, handleResult);
    ...
    $("#heap-histo-search").click(function () {
        /* var now = new Date(dateFormat(new Date())).getTime();
         ...
         if (dateStr) {
             ...
             if (now <= timestamp) {
                 elf.warning("查询时间不能超过当前时间");
                 return;
             }
             if (now - storeDays > timestamp) {
                 elf.warning("不能查询三天前的数据");
                 return;
             }
             getHeapHisto(timestamp);
         } else {
             elf.warning("请选择查询查询时间");
         }*/
        elf.warning("该功能暂时不可用")
    })
    ...
    window.setInterval(function () {
        if (keepRunning) {
            ...
            if (now - startTime < keepTime) {
                ...
                keepRunning = false;
                elf.keepError("运行超过 " + minutes + " 分钟，停止查询", function () {
