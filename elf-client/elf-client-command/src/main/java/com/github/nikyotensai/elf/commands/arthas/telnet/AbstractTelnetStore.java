...
import com.github.nikyotensai.elf.commands.arthas.ArthasEntity;
import com.github.nikyotensai.elf.commands.arthas.TelnetConstants;
...
public abstract class AbstractTelnetStore implements TelnetStore {
    ...
    private Telnet doWithIllegalVersion(Telnet telnet, String version) {
        logger.warn("elf version illegal, current [{}], get [{}]", ElfConstants.CURRENT_VERSION, version);
        try {
            telnet.write(ElfConstants.SHUTDOWN_COMMAND);
        } catch (Exception e) {
            // ignore
        } finally {
            telnet.close();
    ...
    private boolean versionIllegal(String version) {
        return !ElfConstants.CURRENT_VERSION.equals(version);
    }
    ...
    private synchronized TelnetClient doGetTelnet(int pid) {
        ...
        try {
            ...
            resetClient();
            throw new IllegalStateException("can not init elf, " + e.getMessage(), e);
        }
    }
