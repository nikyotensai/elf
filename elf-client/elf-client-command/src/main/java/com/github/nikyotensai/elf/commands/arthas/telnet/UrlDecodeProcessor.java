...
class UrlDecodeProcessor implements ResultProcessor {
    ...
    private final byte[] buf = new byte[CommunicateUtil.DEFAULT_BUFFER_SIZE * 2];
    ...
    public boolean process(byte[] input, int start, int count) {
        Preconditions.checkArgument(count > 0 && count <= CommunicateUtil.DEFAULT_BUFFER_SIZE);
        ...
        length = length + count;
        decode();
        if (isDataEnd || length > CommunicateUtil.DEFAULT_BUFFER_SIZE) {
        ...
        while (index >= start) {
            switch (input[index]) {
                ...
                case '\t':
                    index--;
                    break;
                case CommunicateUtil.LAST_PROMPT_BYTE:
                    return index;
                default:
                    return -1;
            }
