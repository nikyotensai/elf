...
public class ArthasStarter {
    ...
    static {
        String libDirPath = ElfClientConfig.getLibDir();
        File libDir;
        ...
        DEFAULT_AGENT_JAR_PATH = new File(libDir, "elf-instrument-agent.jar").getPath();
    ...
    private static void attachAgent(Configure configure) throws Exception {
        ...
        try {
            ...
            virtualMachine.loadAgent(realAgentFile.getCanonicalPath(),
                    configure.getArthasCore() + delimiter + ";;" + configure.toString() + delimiter + ElfClientConfig.ELF_APP_LIB_CLASS);
        } catch (Exception ex) {
            ex.printStackTrace();
        } finally {
    ...
    private static Configure getConfigure(int pid) {
        String agentJar = System.getProperty("elf.agent.jar.path", DEFAULT_AGENT_JAR_PATH);
        String coreJar = System.getProperty("elf.arthas.core.jar.path", DEFAULT_CORE_JAR_PATH);
